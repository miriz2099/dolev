...
// functions/controllers/message.controller.js
const { db } = require("../config/firebase");
const { sendUrgentMessageEmail } = require("../helpers/mail.helper");

const sendMessage = async (req, res) => {
  try {
    const { receiverId, childId, text, urgent } = req.body;
    const senderId = req.user.uid;
    ...
    // 🆕 הודעה דחופה -> שולחים גם מייל לנמען, בנוסף לשמירה הרגילה במערכת.
    // כשל בשליחת המייל לא אמור להפיל את שליחת ההודעה עצמה - רק מתועד בלוג.
    if (isUrgent) {
      try {
        ...
        if (receiverData?.email) {
          await sendUrgentMessageEmail({
            ...
            therapistFirstName: receiverData.firstName,
        ...
        console.error("Error sending urgent message email:", mailErr);
      }
    }
